...
class Byte:
  ...
  def bitList(self, value, bitNum):
    result = [int(digit) for digit in bin(value)[2:]];
    while (len(result) < bitNum):
        result.insert(0, 0);
    return result;

  def integer(self, bitlist):
    result = int("".join(str(i) for i in bitlist), 2);
    return result;
