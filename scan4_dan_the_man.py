...
def decode_panel_info_reply( packet_to_decode ) :     
    ...
    if ( use_mxspeak_6 == False ) :
        if ( varlist[ 1 ] == 0 ) :
            ...
            print("On network, node " + str( varlist[ 1 ] ) )
            glb_on_a_network = True
...
def FormRequestPacket( node_number, loop, point  ) :

    print( ">>>\nFormRequestPacket()\nnode number " + str( node_number ) + "\n")
    ...
    NextPacket_mxspeak6 = "13,228,0, 0, 0,  1,  0,  0, 0, 150,0, 0, 0 "
...
def PointInformationScan(   ) :
    ...
    while(True):
        ...
        if ( True ) : 
            ...
            packettosend = FormRequestPacket( current_node, current_loop, point_address )
            print( "Sending request for : Node " + str( current_node )+ " Loop " + str( current_loop ) + " Pnt " + str( point_address ) )
            ...
            if length_of_reply > 1: #for some reason printing an empty payload still constitutes to greater than 0, so we use 1 instead
                ...
                if (length_of_reply == 2) :
                    if (( point_address == 1 ) & ( current_loop == 1)) :
                        point_address = constants.MAX_ADDRESSES_PER_LOOP
                ...
                if (length_of_reply > 10) :
                  ...
                  if (( glb_reply_indicates_loop_not_there == True ) | ( glb_reply_indicates_no_more_points_on_loop == True)):
                    glb_reply_indicates_no_more_points_on_loop = False
                    point_address = constants.MAX_ADDRESSES_PER_LOOP
                    print( "\n---------------\nMove to next loop\n----------------\n\n" )
        ...
        #print( "Increment point address here, from " + str( point_address ))
        point_address += 1
        ...
        if (point_address >constants.MAX_ADDRESSES_PER_LOOP):         
            ...
            if (current_loop > max_loop): 
                ...
                if ( glb_on_a_network == True ) :                  
                    ...
                    print( ">>>>>>>>" )
                    print( "Moving to next node " + str(current_node  ))
                    print( ">>>>>>>> point address " +  str( point_address ))
                else:
                    print(">>>>>\nNot on a network so rescan this panel.\n>>>>>>>>>>>>")
